...
### 1. Переход на RabbitMQ

#### Текущий протокол обмена сообщениями
- В настоящее время наша микросервисная архитектура использует **HTTP** для обмена сообщениями между сервисами. Этот 
  протокол выбран за его простоту, масштабируемость и широкую поддержку, что делает его идеальным для начальных этапов 
  проекта.

#### Планируемое обновление до RabbitMQ
- Мы планируем обновить нашу систему обмена сообщениями до **RabbitMQ** для повышения эффективности обработки асинхронных коммуникаций. RabbitMQ обеспечит лучшую устойчивость к сбоям, улучшенную масштабируемость, а также более сложные стратегии маршрутизации и балансировки нагрузки.
- В переходный период будут поддерживаться как HTTP, так и RabbitMQ для обеспечения обратной совместимости.
- Архитектура системы уже продумана с учетом потенциального перехода на RabbitMQ, что упростит интеграцию и обновление.

### 2. Поддержка Gradle 9 и выше

#### Текущая поддержка Gradle
- В настоящее время наш проект тестируется и поддерживается на **версиях Gradle до 8.8**. Эти версии были выбраны из-за их стабильности и широкого использования в наших средах разработки.

#### Планируемая поддержка Gradle 9
- Мы знаем о предстоящих изменениях в **Gradle 9** и необходимости адаптировать наши сборочные скрипты для соответствия новым требованиям, исключающим изменение разрешенных конфигураций.
- Мы планируем обновить наш проект для поддержки Gradle 9 и выше, чтобы обеспечить совместимость с будущими версиями Gradle и использовать новые функции и улучшения, которые они предлагают.
